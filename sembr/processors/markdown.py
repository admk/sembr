...
class MarkdownProcessor(BaseProcessor):
    ...
    def _find_inline_text_nodes(
        self, node, text: str, regions: List[Dict[str, Any]],
        context_stack=None
    ):
        ...
        if node.type == "inline":
            ...
            if text_content.strip():  # Only process non-empty content
                ...
                continuation_info = self._get_continuation_format(
                    new_context_stack, text
                )
                regions.append(
                    {
                        "text": text_content,
        ...
        for child in node.children:
            self._find_inline_text_nodes(
                child, text, regions, new_context_stack)

    def _get_continuation_format(
        self, context_stack: List[Dict[str, Any]], text: str
    ) -> Dict[str, str]:
    ...
    def generate(self, paragraphs: List[Dict[str, Any]], join: bool = True) -> str:
        ...
        sorted_regions = sorted(
            paragraphs, key=lambda x: x["start_byte"], reverse=True)
        # Replace each inline text region with wrapped version
        for region in sorted_regions:
            processed_text = self._generate_wrapped_text(region)
            prefix = result_text[:region["start_byte"]]
            suffix = result_text[region["end_byte"]:]
            result_text = prefix + processed_text + suffix
        return result_text
